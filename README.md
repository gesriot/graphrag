...
plan against the live graph, planning a direct snapshot
transfer between two managed graphs, or applying that transfer
is an
...
This is directed mutual-reachability grouping only: not weak components,
Leiden or semantic community detection, architecture, hierarchy,
...
exposes that existing bounded producer as `impact_graph`, the 18th
read-only tool, immediately after legacy `impact` and immediately
before `type_closure`. There is no hyphenated alias. Envelope `data`
is the complete producer mapping. Envelope `total` /
`returned` are node-plus-edge counts; `truncated` is
`nodes_truncated or edges_truncated`. Limits are the dedicated
impact-graph defaults 3/50/100 and hard maxima 32/500/500, plus
`max_envelope_bytes = 1_000_000`. The 1 MiB envelope fails closed
with no secondary shrinking. Explicit snapshot ids never activate or
consult `current`. The shared reader lease is held through producer
execution, JSON-ready conversion, envelope construction, size check,
and return. MCP does not expose DOT or output-format selection; CLI
and Python remain the visualization surfaces. Legacy `impact` remains
a separate unbounded title-list contract. This is not the unbounded
`impact`
title list, runtime
execution proof, complete dynamic dispatch, call-observation
reconstruction, semantic impact, severity, ownership, importance,
...
to delete anything. `snapshot-export-verify --export-dir <directory>
--expected-export-revision sha256:<hex>` checks that one already-created
standalone export directory still contains exactly that envelope. It
does not inspect a managed graph, acquire a graph lease, or mutate
the export. Bytes and structure only: it is not a backup,
authenticity, or recoverability claim. `snapshot-export-reconcile
--plan-file <saved-plan.json> --destination <path>` observes one
standalone destination after a complete, partial, interrupted, or
externally modified export attempt. It does not inspect a managed
graph, acquire a graph lease, or mutate the destination. Absence does
...
is a separate read-only schema-2 classification of those
same leftovers. Cleanup-plan schema 1 was read-only/pre-apply
(`apply_supported=false`) and is not accepted by apply. Schema
2 sets `apply_supported=true` and keeps `cleanup_applied=false`.
A deletion candidate is reported only for a
...
delete. The plan does not accept an expected revision or
confirmation. `snapshot-export-staging-cleanup --parent
<directory> --expected-plan-revision sha256:<hex>
--cleanup-confirmed` is the separate CAS apply. There is no
dry-run; the plan command is the preview. Confirmation is
required even when the candidate set is empty. Apply anchors
the selected parent, recomputes the schema-2 plan, compares
the caller token, nonblockingly claims every selected existing
writer lock, revalidates parent/staging/lock identities, and
only then deletes. There is no graph lease because this
operates on an arbitrary export parent. Recursive deletion is
not transactionally atomic. A partial result reports
`partial=true` and always requires a fresh schema-2 plan;
there is no rollback, trash, quarantine, or recovery. Advisory
locks protect only cooperating processes. Non-cooperating
processes remain outside the protection boundary. No
ownership, liveness, backup, authenticity, or recovery is
claimed. `snapshot-export-staging-cleanup-reconcile --parent
<directory> --plan-file <saved-cleanup-plan.json>` is the
separate observation-only aftermath inspection of that saved
schema-2 plan and an optional saved schema-1 apply result.
It does not mutate, claim a writer lease, inspect a managed
graph, or emit a retry token. Absence does not prove apply
deleted an entry; presence does not prove apply failed;
matching identity does not prove ownership or continuous
identity. A fresh schema-2 cleanup plan is required before
any later apply. `snapshot-import-plan --graph <root>
--export-dir <directory>` is a read-only plan for adding one
standalone snapshot export to an existing managed graph. It
anchors and validates the export, holds its descriptors while it
observes the target under one shared existing-lock lease, then
rechecks the complete source under that lease. It validates the
language-independent stored snapshot envelope and reports whether
that snapshot id is
...
`snapshot-import-reconcile --graph <root> --plan-file
<saved-import-plan.json>` is the separate observation-only
aftermath inspection of that saved schema-1 plan and an optional
saved schema-1 apply result. It does not retry, recover, copy,
...
accepted only by `snapshot-transfer-apply` after that command
freshly reproduces the same plan. A ready plan does not authorize
a later apply without freshly reproducing the complete plan and
matching `transfer_revision`.
`snapshot-transfer-apply --source-graph <root> --snapshot
<id|current> --target-graph <root> --expected-transfer-revision
sha256:<hex> --transfer-confirmed` is the CAS apply. It holds one
source-shared and one target-exclusive existing-lock lease in the
same global path-byte order, copies exact source bytes into
`.staging-<id>`, holds `.staging-writer.lock` through copying, then
...
- `scripts/byog_snapshot_graph_audit.py` – **read-only language-independent
  ...
  export. A stable revision mismatch emits the report and exits 1;
  unsafe structure exits 1 with empty stdout.
  ...
  anything. Schema 1 was read-only/pre-apply and is not
  accepted by apply. Schema 2 sets `apply_supported=true`.
  `not_held_at_scan` is not permission to delete.
  `graphrag-code snapshot-export-staging-cleanup --parent
  <directory> --expected-plan-revision sha256:<hex>
  --cleanup-confirmed` is the CAS apply. There is no dry-run
  and no graph lease. Per-candidate cooperative writer claims
  are required before the first deletion. Recursive deletion
  is not transactionally atomic. Advisory
  locks protect
  ...
  observes that parent against the saved schema-2 plan and
  optional saved apply result. It does not recover, retry,
  mutate, or prove that apply deleted or failed to delete a
  name. A fresh cleanup plan is required before any later
  apply. `graphrag-code snapshot-import-plan --graph <root>
  ...
  --plan-file <saved-import-plan.json>` observes that graph
