...
def test_python_module_help():
    ...
    assert "snapshot-export-apply" in proc.stdout
    assert "snapshot-export-verify" in proc.stdout
    assert "snapshot-export-reconcile" in proc.stdout
    ...
    assert "snapshot-import-plan" in proc.stdout
    assert "snapshot-import-apply" in proc.stdout
...
def test_wheel_and_sdist_contents(built_wheel_and_sdist):
    ...
    with zipfile.ZipFile(wheel) as zf:
        ...
        assert "graphrag_code/degree_ranking_dot.py" in names
        assert "graphrag_code/dependency_order_dot.py" in names
        assert "graphrag_code/type_closure_dot.py" in names
        assert "graphrag_code/impact_graph_dot.py" in names
        ...
        assert "graphrag_code/snapshot_export_apply.py" in names
        ...
        assert "graphrag_code/snapshot_import_plan.py" in names
        assert "graphrag_code/snapshot_import_apply.py" in names
        assert "graphrag_code/snapshot_import_reconcile.py" in names
        assert "graphrag_code/snapshot_transfer_plan.py" in names
        assert "graphrag_code/snapshot_transfer_apply.py" in names
    ...
    assert any(n.endswith("graphrag_code/degree_ranking_dot.py") for n in snames)
    assert any(n.endswith("graphrag_code/dependency_order_dot.py") for n in snames)
    assert any(n.endswith("graphrag_code/type_closure_dot.py") for n in snames)
    assert any(n.endswith("graphrag_code/impact_graph_dot.py") for n in snames)
