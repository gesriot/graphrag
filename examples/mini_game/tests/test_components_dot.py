"""Deterministic Graphviz DOT export for the bounded components CLI.

Renders the existing ByogGraph.components result. Does not add a Graphviz
runtime, a second weak-components algorithm, an MCP format parameter, or
an output file.
"""
...
from scripts.graph_query import (  # type: ignore
    dumps_components_dot,
    dumps_components_json,
    format_components_human,
)
from graphrag_code.components_dot import (  # type: ignore
    COMPONENTS_DOT_SCHEMA_VERSION,
    HARD_MAX_COMPONENTS_DOT_BYTES,
    ComponentsDotError,
    dumps_components_dot as direct_dumps,
)
...
graph graphrag_components {
  ...
  subgraph cluster_c0001 {
    ...
    nodes_truncated="false";
    n0003 [label="C", title="C"];
    n0004 [label="Zsmall", title="Zsmall"];
  }
  subgraph cluster_c0002 {
    ...
    n_endpoint_only_nodes="0";
    nodes_truncated="false";
    n0005 [label="Isolated", title="Isolated"];
  }
}
"""
...
graph graphrag_components {
  ...
  subgraph cluster_c0000 {
    ...
    nodes_truncated="false";
    n0000 [label="A", title="A"];
    n0001 [label="M", title="M"];
    n0002 [label="Z", title="Z"];
  }
  subgraph cluster_c0001 {
    label="B";
    component_id="c0001";
...
def _publish(tmp_path: Path, entities: list, relationships: list) -> Path:
    graph = tmp_path / "byog_components_dot"
    texts = [
        {
            "id": f"tu:{e['title']}",
...
def _empty_result(**overrides: object) -> dict:
    result = compute_weakly_connected_components(pd.DataFrame(), pd.DataFrame())
    result.update(copy.deepcopy(overrides))
    return result


def test_golden_multi_empty_unicode_and_hash_seed_independent():
    payload = dumps_components_dot(_base_result())
    assert payload == GOLDEN_MULTI
    assert payload == dumps_components_dot(_base_result())
    assert payload == direct_dumps(_base_result())
    assert payload.endswith("\n") and not payload.endswith("\n\n")
    assert payload.encode("utf-8").decode("utf-8") == payload
    assert (
        "schema_version=" + quote_dot_string(str(COMPONENTS_DOT_SCHEMA_VERSION))
        in payload
    )
    assert dumps_components_dot(_empty_result()) == GOLDEN_EMPTY
    ents = pd.DataFrame(_unicode_entities())
    rels = pd.DataFrame(_unicode_rels())
    shuffled = dumps_components_dot(
        compute_weakly_connected_components(
    ...
    assert dumps_components_dot(compute_weakly_connected_components(ents, rels)) == (
        GOLDEN_UNICODE
    )
    code = r"""
import pandas as pd
from graphrag_code.byog_graph import compute_weakly_connected_components
...
ents = pd.DataFrame([
    ...
    {"id": "eZ", "title": "Zsmall", "type": "function", "source_file": "a.py", "extractor": "x"},
])
rels = pd.DataFrame([
    {"id": "r1", "source": "A", "target": "B", "type": "calls", "extractor": "x"},
    {"id": "r2", "source": "B", "target": "ghost", "type": "calls", "extractor": "x"},
    {"id": "r3", "source": "A", "target": "A", "type": "calls", "extractor": "x"},
    {"id": "r4", "source": "A", "target": "B", "type": "calls", "extractor": "x"},
    {"id": "r5", "source": "C", "target": "Zsmall", "type": "contains", "extractor": "x"},
])
print(dumps_components_dot(compute_weakly_connected_components(ents, rels)), end="")
"""
    payloads = []
...
def test_live_graph_preserves_clusters_ids_and_has_no_edges(tmp_path: Path):
    graph = _publish(tmp_path, _multi_entities(), _multi_rels())
    result = ByogGraph(graph).components()
    payload = dumps_components_dot(result)
    assert payload == GOLDEN_MULTI
    ...
    for component, cluster in zip(result["components"], _cluster_ids(payload)):
        assert component["representative"] not in cluster
    structural = _structural(payload)
    assert " -> " not in structural
    assert " -- " not in structural
    assert "Isolated" not in structural.split("subgraph")[0]
    leaky = _base_result()
    ...
    rendered = dumps_components_dot(leaky)
    assert "should-not-appear" not in rendered
    assert "component-secret" not in rendered
    assert "secret" not in rendered


def test_truncation_flags_and_empty_graph(tmp_path: Path):
    graph = _publish(tmp_path, _multi_entities(), _multi_rels())
    g = ByogGraph(graph)
    components = dumps_components_dot(g.components(max_components=1))
    assert 'n_components_total="3"' in components
    ...
    assert 'label="A"' in components
    assert "cluster_c0001" not in components
    assert "Isolated" not in components
    assert " -> " not in components
    assert " -- " not in components

    nodes = dumps_components_dot(g.components(max_nodes_per_component=1))
    assert 'nodes_truncated="true"' in nodes
    assert 'components_truncated="false"' in nodes
    assert _cluster_ids(nodes) == ["cluster_c0000", "cluster_c0001", "cluster_c0002"]
    assert _node_ids(nodes) == ["n0000", "n0001", "n0002"]
    assert 'n_nodes_total="3"' in nodes
    assert 'n_nodes_returned="1"' in nodes
    assert 'label="A"' in nodes
    assert "ghost" not in nodes
    assert "Zsmall" not in nodes
    assert " -> " not in nodes

    empty = dumps_components_dot(
        compute_weakly_connected_components(pd.DataFrame(), pd.DataFrame())
    )
...
def test_escaping_injection_controls_and_no_new_statements():
    ...
    controlled = dumps_components_dot(controls)
    ...
    assert "\x9f" not in controlled
    assert "\x85" not in controlled
    cr = _base_result(
        components=[
            {
                "representative": "A\rB",
    ...
    cr_payload = dumps_components_dot(cr)
...
def test_missing_malformed_invariants_and_lone_surrogate_fail_closed():
    for name in (
        ...
        del missing[name]
        with pytest.raises(ComponentsDotError, match="missing"):
            dumps_components_dot(missing)
    with pytest.raises(ComponentsDotError):
        dumps_components_dot("not-a-mapping")  # type: ignore[arg-type]
    ...
    with pytest.raises(ComponentsDotError, match="UTF-8"):
        dumps_components_dot(surrogate)
    ...
    with pytest.raises(ComponentsDotError, match="duplicate"):
        dumps_components_dot(duplicate)
    overlap = _base_result()
    ...
    with pytest.raises(ComponentsDotError, match="duplicate"):
        dumps_components_dot(overlap)
    out_of_order = _base_result(
        components=list(reversed(_base_result()["components"]))
    )
    with pytest.raises(ComponentsDotError, match="canonical producer order"):
        dumps_components_dot(out_of_order)
    unsorted_nodes = _base_result()
    unsorted_nodes["components"][0] = {
        **unsorted_nodes["components"][0],
        "nodes": ["ghost", "A", "B"],
        "representative": "ghost",
    }
    with pytest.raises(ComponentsDotError, match="UTF-8 byte order"):
        dumps_components_dot(unsorted_nodes)
    bad_rep = _base_result()
    bad_rep["components"][0] = {
        **bad_rep["components"][0],
        "representative": "B",
    }
    with pytest.raises(ComponentsDotError, match="first returned node"):
        dumps_components_dot(bad_rep)
    step_mismatch = _base_result()
    step_mismatch["components"][0] = {
        **step_mismatch["components"][0],
        "n_nodes_returned": 2,
        "nodes": ["A", "B"],
    ...
    step_mismatch["max_nodes_per_component"] = 2
    step_mismatch["nodes_truncated"] = True
    # n_nodes_returned 2 != max 2 is ok; representative must still be first.
    dumps_components_dot(step_mismatch)
    wrong_trunc_cap = copy.deepcopy(step_mismatch)
    wrong_trunc_cap["components"][0] = {
        **wrong_trunc_cap["components"][0],
        "nodes": ["A"],
        "n_nodes_returned": 1,
    }
    with pytest.raises(ComponentsDotError, match="exactly max_nodes_per_component"):
        dumps_components_dot(wrong_trunc_cap)
    for invalid in (
        ...
        _base_result(max_nodes_per_component=1.5),
        _base_result(n_nodes_total=math.nan),
        _base_result(max_components=HARD_MAX_COMPONENTS + 1),
        _base_result(max_nodes_per_component=HARD_MAX_COMPONENT_NODES + 1),
        _base_result(n_nodes_total=1.0),
        _base_result(components_truncated=1),
        _base_result(n_edges_total=-1),
        _base_result(n_entity_nodes_total=4, n_endpoint_only_nodes_total=1),
        ...
        _base_result(n_edges_total=4),
    ):
        with pytest.raises(ComponentsDotError):
            dumps_components_dot(invalid)
...
def test_truncated_aggregate_invariants_fail_closed():
    ...
    impossible_nodes.update(
        n_nodes_total=3,
        n_entity_nodes_total=2,
        n_endpoint_only_nodes_total=1,
    )
    with pytest.raises(ComponentsDotError, match="omitted component"):
        dumps_components_dot(impossible_nodes)
    too_many_edges = copy.deepcopy(truncated)
    too_many_edges.update(n_edges_total=3)
    with pytest.raises(ComponentsDotError, match="exceed"):
        dumps_components_dot(too_many_edges)
    uncovered = _base_result(n_edges_total=99)
    with pytest.raises(ComponentsDotError, match="cover n_edges_total"):
        dumps_components_dot(uncovered)


def test_edge_types_none_literal_all_and_commas():
    unfiltered = dumps_components_dot(_base_result(edge_types=None))
    literal_all = dumps_components_dot(_base_result(edge_types=["all"]))
    comma_types = dumps_components_dot(_base_result(edge_types=["a,b", "c"]))
    split_types = dumps_components_dot(_base_result(edge_types=["a", "b,c"]))
...
def test_byte_cap_boundaries(monkeypatch: pytest.MonkeyPatch):
    ...
    import graphrag_code.components_dot as dot_mod

    monkeypatch.setattr(dot_mod, "HARD_MAX_COMPONENTS_DOT_BYTES", size)
    assert dumps_components_dot(_base_result()) == payload
    monkeypatch.setattr(dot_mod, "HARD_MAX_COMPONENTS_DOT_BYTES", size + 1)
    assert dumps_components_dot(_base_result()) == payload
    monkeypatch.setattr(dot_mod, "HARD_MAX_COMPONENTS_DOT_BYTES", size - 1)
    with pytest.raises(ComponentsDotError, match="hard limit"):
        dumps_components_dot(_base_result())
    assert HARD_MAX_COMPONENTS_DOT_BYTES == 1_000_000
...
def test_cli_json_dot_exclusive_human_json_unchanged_and_overflow(
    ...
    graph = _publish(tmp_path, _multi_entities(), _multi_rels())
    common = ["components", "--graph", str(graph)]
    ...
    assert dumps_components_json(payload) + "\n" == json_out.stdout
    assert format_components_human(payload).strip() == human.stdout.strip()
    ...
    rejected = CliRunner().invoke(
        app, ["components", "--graph", str(graph), "--json", "--dot"]
    )
    assert rejected.exit_code == 2
    assert rejected.stdout == ""
    monkeypatch.undo()
    monkeypatch.setattr(dot_mod, "HARD_MAX_COMPONENTS_DOT_BYTES", 32)
    overflow = CliRunner().invoke(app, ["components", "--graph", str(graph), "--dot"])
...
def test_cli_help_mentions_dot_and_not_an_image():
    for args in (
        [sys.executable, str(QUERY), "components", "--help"],
        [sys.executable, str(CLI), "components", "--help"],
        [sys.executable, "-m", "graphrag_code.graph_query", "components", "--help"],
        [sys.executable, "-m", "graphrag_code", "components", "--help"],
    ):
        help_out = _run(*args)
...
def test_script_module_product_installed_dot_parity(
    ...
    graph = _publish(tmp_path, _multi_entities(), _multi_rels())
    args = ["components", "--graph", str(graph), "--dot"]
    ...
    expected = dumps_components_dot(ByogGraph(graph).components())
    assert (
        script.stdout
        == module.stdout
        == product.stdout
...
def _components_dot_hold(graph: str, held, resume, q) -> None:
    sys.path.insert(0, str(ROOT / "src"))
    import sys as sys_mod

    from graphrag_code import graph_query

    orig_dumps = graph_query.dumps_components_dot
    ...
    graph_query.dumps_components_dot = wrap_dumps
    ...
    class HoldingStdout:
        ...
        def write(self, data):
            ...
            if isinstance(data, str) and data.startswith("graph "):
...
def test_publisher_waits_through_dot_render_and_stdout_flush(tmp_path: Path):
    ...
    q = CTX.Queue()
    reader = CTX.Process(
        target=_components_dot_hold, args=(str(graph), held, resume, q)
    )
...
def test_no_nested_query_graphviz_or_tempfiles_and_single_producer_call(
    ...
    monkeypatch.setattr(g, "components", wrapped)
    for name in (
        "condensation",
        "strong_components",
        "shortest_path",
    ...
    dumps_components_dot(result)
    assert calls == ["components"]

    serializer_calls = 0
    orig_dumps = dumps_components_dot
    ...
    monkeypatch.setattr(gq, "dumps_components_dot", counted_dumps)
    invoked = CliRunner().invoke(
        gq.app, ["components", "--graph", str(graph), "--dot"]
    )
    assert invoked.exit_code == 0, invoked.stderr
    assert serializer_calls == 1
    assert invoked.stdout == GOLDEN_MULTI

    dot_src = (ROOT / "src" / "graphrag_code" / "components_dot.py").read_text(
        encoding="utf-8"
    )
    ...
    for node in gq_tree.body:
        if isinstance(node, ast.FunctionDef) and node.name == "cli_components":
            components_fn = ast.get_source_segment(gq_src, node)
    assert components_fn is not None
    assert "subprocess" not in components_fn
    assert "compute_weakly_connected_components" not in components_fn
    assert "networkx" not in components_fn
    assert components_fn.count(".components(") == 1
    assert components_fn.count("dumps_components_dot(") == 1
    assert "sys.stdout.write(dumps_components_dot(result))" in components_fn
    assert "sys.stdout.flush()" in components_fn
    write_at = components_fn.find("sys.stdout.write(dumps_components_dot(result))")
    flush_at = components_fn.find("sys.stdout.flush()")
    with_at = components_fn.find("with _scoped_graph")
    ...
    async def _body():
        async with Client(server) as client:
            ...
            if isinstance(body, dict) and set(body) == {"result"}:
                body = body["result"]
            assert body["tool"] == "components"
            dumped = json.dumps(body)
            assert "digraph" not in dumped
            assert "graphrag_components" not in dumped
