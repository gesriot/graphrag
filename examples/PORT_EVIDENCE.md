...
| `sqlparse` | Python split/lex contract → inherited-member and direct-initializer runtime audits → fresh Python graph/context packs → Rust contract | Complete Rust port; direct `__init__.py` definitions are graph entities, while bare re-export aliases stay an explicit residual; a separate local-graph call oracle measures the 65-case source corpus |
