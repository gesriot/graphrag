...
- **Exhaustive codec/CD parity**: 6 pytest items covering all supported single-byte `encoding_languages`, multibyte language mapping, all 0x00..0xFF strict decode probes for single-byte codecs, representative multibyte strict decode probes, and representative encode/output round-trips. The latter includes every one of Python HZ's 7,445 valid shifted pairs in both strict directions plus replacement-mode U+20AC. Current status: 4 pass + 2 expected xfails for UTF-7 (SIG/BOM strip policy per api.py vs raw stdlib) and euc_jis_2004 (extension vs encoding_rs profile). Single-byte codecs and HZ are exact; most other MB paths use encoding_rs or custom Korean/UTF handling, with rare table variants documented.
...
The legacy post-processing (small-sample confidence adjustment, `utf_8_sig` mapping, and `CHARDET_CORRESPONDENCE`) is implemented only in `detect_legacy*`. The detection codec contract matches Python `api.py` (SIG/BOM stripping per `should_strip_sig_or_bom` except UTF-16/32; UTF-7 special full-decode-then-strip). Raw stdlib `codecs.decode` may differ for UTF-7 (keeps U+FEFF) and rare MB table variants (big5*/euc_jis_2004 extensions). HZ no longer uses the legacy crate's GBK-like table: `tools/generate_codecs.py` derives its exact 7,445-pair Python HZ/GB2312 map, tested in both strict directions and in replacement mode. Rust uses custom handling only for utf7/hz/johab/iso2022_kr plus encoding_rs for most other MB. See `KNOWN_XFAIL` in the parity test.
...
- Default test surface includes exhaustive CD and single-byte codec probes, every valid generated Python HZ shifted pair, representative other-multibyte probes, and the 79-input seeded live detector corpus; the 530-input full corpus is opt-in. This is not an unbounded random fuzzer or exhaustive multibyte variant table. Single-byte codecs and HZ are exact via generated tables; most other multibyte paths use encoding_rs (with documented rare table/extension differences for big5*/euc_jis_2004/iso2022_jp* variants versus Python stdlib codecs).
