...
The legacy post-processing (small-sample confidence adjustment, `utf_8_sig` mapping, and `CHARDET_CORRESPONDENCE`) is implemented only in `detect_legacy*`. The detection codec contract matches Python `api.py` (SIG/BOM stripping per `should_strip_sig_or_bom` except UTF-16/32; UTF-7 special full-decode-then-strip). Raw stdlib `codecs.decode` may differ for UTF-7 (keeps U+FEFF) and rare MB table variants (big5*/euc_jis_2004 extensions); the HZ replacement encoder also has a known U+20AC GBK-vs-Python-GB2312 table difference, recorded in `API_SURFACE_AUDIT.md` as out of scope for the generated codec table. Rust uses custom handling only for utf7/hz/johab/iso2022_kr plus encoding_rs for most other MB. See `KNOWN_XFAIL` in the parity test.
