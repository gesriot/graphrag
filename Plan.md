...
3. Decide the next frontier:
  ...
  - a deliberate cJSON representation/policy redesign (shared/borrowed nodes or handles for the 8 traced aliases/identity calls; allocator/error state for the 4 traced global calls), or locale/partial-`strtod` work, if the goal is beyond the closed exclusive-ownership API surface;
