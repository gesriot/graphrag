...
- C/C++ specifics:
  ...
  - Bootstrap status (2026-06-25): vendored MIT `jsmn` indexes via `scripts/extract_c.py` / `scripts/index_c.py` using tree-sitter-c. The resulting graph has 30 entities, 68 relationships, 157 call observations, 40 resolved calls, and `audit_call_edges` pass rate 1.0 with 0 anomalies/dangling/semantic suspicions. This proves the audit rail is frontend-agnostic at graph level, but not yet C→Rust porting.
...
1. Treat `PHASE5_REPORT.md` + `sqlparse.split` as the frozen Phase 5 evidence baseline, and `examples/jsmn/PROVENANCE.md` as the Phase 6 frontend bootstrap baseline.
2. Proceed to the first bounded C→Rust pilot on `jsmn`: capture a golden contract before Rust (`jsmn_parse` inputs → return code + token sequence: type/start/end/size, with strict/non-strict scope stated), then port only that bounded API.
3. Keep C-specific scope explicit: default macro configuration from `compile_commands.json`, no claim yet for clang-accurate macro/type facts, include relationships, function-pointer resolution, or multiple build configurations.
4. Add C port verification rails before implementation: compile/run upstream C tests if possible, generate golden JSON from the vendored C implementation, add Rust contract tests, then run `port_eval` with C graph/context packs and manual-fix count.
5. After `jsmn` is green, move to `inih` for callbacks/file I/O; reserve `cJSON` for struct/pointer ownership once the C rail has one successful port.
