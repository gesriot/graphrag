...
- C/C++ specifics:
  ...
  - cJSON update (2026-07-25): the same gate now covers owned mutation/builder and structural operations — 59 golden cases (22 parse ownership + 30 float print + 7 mutation traces), C ASan over all mutation traces, and a safe Rust `Box`-transfer API for constructors, typed arrays, add/insert/detach/delete/replace, duplicate, compare, and value accessors. Reference/alias and pointer-identity calls remain outside this exclusive-ownership port, so full C API migration is still not claimed.
  - Preprocessor *detection* (2026-07-26): `scripts/c_preprocessor.py` (no clang) stamps `preprocessor_dependent` / `preprocessor_reasons` on entities, call edges, and observations from source text plus `compile_commands.json` — non-guard `#if`/`#ifdef` regions, function-like macros, `-D` defines, and entity bodies containing directives. Labels are provenance only: they do not flip `is_deterministic`, drop edges, or change audit pass rates. Published C graphs now carry the columns (`byog_jsmn` / `byog_inih` reindexed with structure unchanged; `byog_cjson` stamped in place so the pinned 239 call count is not mixed with a later runner expansion that would rebuild to 495). Context packs surface a top-level `preprocessor_warning` (e.g. `ini:ini_parse_stream`). This is **not** macro expansion, include resolution, typedef-chain resolution, or config-aware dead-branch elimination — those still require clang over `compile_commands.json`.
...
2. Keep C-specific scope explicit: preprocessor *dependence* is now labelled on published C graphs and in context packs (detection only), but there is still no claim for clang-accurate macro expansion, type facts, include relationships, full function-pointer resolution, multiple build configurations, full C ABI preservation, custom allocators/hooks, or full cJSON mutation API coverage. Default macro/build configuration continues to come from `compile_commands.json` metadata for the diagnostic; evaluating those defines into a single configuration remains clang territory.
3. Decide the next frontier:
   - finish any remaining scoped `charset-normalizer` hardening (additional corpus seeds, exhaustive multibyte variant tables, scale harness runs, or additional upstream API compatibility) if the goal is to turn the stress-test port into a stronger product artifact;
   - remaining cJSON depth (reference/alias and pointer-identity APIs, custom hooks, locale decimal points) if the goal is fuller API coverage beyond the ownership + print + owned-mutation + Miri/props checkpoint;
...
*Plan created: 2026-06-14. Reviewed/updated: 2026-07-25. Current implementation strategy: prove deterministic BYOG + local context packs on increasingly realistic Python targets, including data-heavy heuristic libraries, before expanding to C/C++, semantic overlays, or product packaging.*
