...
call-oracle effect. The current full-suite expectation is **616 passed, 2 xfailed**;
...
2. Keep C-specific scope explicit: preprocessor *dependence* is labelled on published C graphs (detection only); optional compiler overlays can add flattened TU `depends_on` and direct `includes` when explicitly enabled. A **Clang AST function-definition audit** (`scripts/c_clang_ast_audit.py`) is available as a diagnostic evidence gate (signatures/locations vs tree-sitter; not a graph overlay). There is still no claim for published clang-derived type/signature *graph facts*, full macro expansion fidelity, full function-pointer resolution, multi-config coverage, full C ABI preservation, or a full upstream cJSON migration beyond its closed exclusive-ownership surface.
