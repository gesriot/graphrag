...
1. Treat `PHASE5_REPORT.md` + `sqlparse.split` as the frozen Phase 5 evidence baseline, and `examples/jsmn/PROVENANCE.md` as the Phase 6 first-C-port baseline.
2. Move to `inih` as the next bounded C target: license/provenance → compile metadata → C graph audit → golden contract before Rust → port only the parser/callback surface.
3. Use `inih` to measure the next C-specific unknowns separately from ownership-heavy translation: callback invocation edges, file/string input variants, line/section/name/value lifetime semantics, error/line-number behavior, and compile-time options.
4. Keep C-specific scope explicit: default macro/build configuration from `compile_commands.json`, no claim yet for clang-accurate macro/type facts, include relationships, function-pointer resolution, multiple build configurations, or full C ABI preservation.
