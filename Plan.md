...
call-oracle effect. The current full-suite expectation is **720 passed, 2 xfailed**;
this 2026-08-08 update supersedes the earlier 2026-07-26 gate snapshot.
...
2. Keep C-specific scope explicit: preprocessor *dependence* is labelled on published C graphs (detection only); optional compiler overlays can add flattened TU `depends_on`, direct `includes`, (via `--clang-signatures`) configuration-derived Clang function-signature *fields* on matched tree-sitter function entities, and (via `--clang-calls`) configuration-derived direct-call *evidence fields* on existing tree-sitter `calls` relationships (exact span + byte-offset attachment; no new edges). When both Clang overlays are enabled they share one in-memory AST capture (one dump per compile entry; no disk cache). Tree-sitter C symbol titles are kind-aware under cross-kind collisions (`module_key:entity_kind:name` for colliding function/struct/enum/typedef pairs; legacy `module_key:name` otherwise) so silent title-only collapse of `typedef struct T {…} T;` no longer discards the named struct. Typedef aliases nested in declarators (function-pointer typedefs included) are extracted by walking only declarator structure; configuration-mutually-exclusive declaration sites (e.g. inih `ini_handler` under `#if/#else`) remain exact-identity residuals until a focused configuration-aware selection slice. Standalone AST diagnostics remain available: **function-definition audit** (`scripts/c_clang_ast_audit.py`), **call-site audit** (`scripts/c_clang_call_audit.py`), and **type-declaration audit** (`scripts/c_clang_type_audit.py`, diagnostic only — collision-safe kind/path/name/line/col identity; **no** `uses_type` graph overlay, no `index_c` flag). Signature and call publication are independent fail-closed opt-ins (default off); unclean call residuals abort rather than inventing facts. There is still no claim for full type resolution, type-use graphs, ABI verification, function-pointer target resolution (points-to), multi-config coverage, full C ABI preservation, or a full upstream cJSON migration beyond its closed exclusive-ownership surface.
