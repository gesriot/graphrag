...
3. Decide the next frontier:
  ...
  - a deliberate cJSON representation/policy redesign (shared mutable bytes/nodes or a handle arena for the 6 compiler-proven safe-borrow alias cases; allocator/error state for the 4 traced global calls), or locale/partial-`strtod` work, if the goal is beyond the closed exclusive-ownership API surface;
