...
call-oracle effect. The current full-suite expectation is **647 passed, 2 xfailed**;
...
2. Keep C-specific scope explicit: preprocessor *dependence* is labelled on published C graphs (detection only); optional compiler overlays can add flattened TU `depends_on`, direct `includes`, and (via `--clang-signatures`) configuration-derived Clang function-signature *fields* on matched tree-sitter function entities. Standalone AST diagnostics remain separate: **function-definition audit** (`scripts/c_clang_ast_audit.py`) and **call-site audit** (`scripts/c_clang_call_audit.py`, evidence gate before any future configured call overlay). Signature publication is fail-closed opt-in; call audit publishes **no** graph facts. There is still no claim for full type resolution, ABI verification, function-pointer target resolution, multi-config coverage, full C ABI preservation, or a full upstream cJSON migration beyond its closed exclusive-ownership surface.
