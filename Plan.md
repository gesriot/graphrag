...
2. Port the captured `cJSON` ownership slice to Rust: parse → inspect tree → print → delete over the bounded JSON corpus, matching the unformatted/inspect/formatted oracles.
3. Use the `cJSON` port to measure the next C-specific unknowns separately: struct field modeling, pointer ownership, allocation/free behavior, recursive tree shape, string ownership, and API error/null semantics.
