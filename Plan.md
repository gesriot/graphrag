...
call-oracle effect. The current full-suite expectation is **637 passed, 2 xfailed**;
...
2. Keep C-specific scope explicit: preprocessor *dependence* is labelled on published C graphs (detection only); optional compiler overlays can add flattened TU `depends_on`, direct `includes`, and (via `--clang-signatures`) configuration-derived Clang function-signature *fields* on matched tree-sitter function entities. The **Clang AST function-definition audit** (`scripts/c_clang_ast_audit.py`) remains the standalone diagnostic evidence gate; signature publication is a separate fail-closed opt-in. There is still no claim for full type resolution, ABI verification, macro expansion fidelity, function-pointer resolution, multi-config coverage, full C ABI preservation, or a full upstream cJSON migration beyond its closed exclusive-ownership surface.
