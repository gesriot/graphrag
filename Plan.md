...
2. For `cJSON`, capture the golden contract before Rust: parse → inspect tree → print → delete over a bounded JSON corpus, with ASan-enabled C runner when available.
3. Use the `cJSON` golden to measure the next C-specific unknowns separately: struct field modeling, pointer ownership, allocation/free behavior, recursive tree shape, string ownership, and API error/null semantics.
