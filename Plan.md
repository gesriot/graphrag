...
- C/C++ specifics:
  ...
  - cJSON update (2026-07-25): the same gate now covers its owned mutation/builder extension — 56 golden cases (22 parse ownership + 30 float print + 4 mutation traces), C ASan over all mutation traces, and a safe Rust `Box`-transfer API for constructors, typed arrays, add, detach, delete, and replace. Reference/alias and pointer-identity calls remain outside this exclusive-ownership port, so full C API migration is still not claimed.
