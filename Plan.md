...
2. Capture the behavior contract before Rust for the first scaled component. Preferred scope: the `sqlparse.split` pipeline (`__init__.split` → `FilterStack` → `lexer.tokenize` → `StatementSplitter` → optional semicolon stripping → `sql.Statement` stringification). This exercises real cross-module flow without taking on the full formatter/grouping surface at once.
3. Build a differential SQL corpus before porting: semicolon splitting, `GO`, `BEGIN`/`END`, `CASE`, nested parentheses, comments/whitespace, string literals, bytes/encoding cases, strip-semicolon mode, and representative malformed/edge inputs. Run it against vendored Python first and freeze expected traces.
4. Port the scoped component through the existing gates. Record context-pack inputs, iterations, failures, engineer time, manual-fix count, and behavioral deltas; add property/fuzz tests where the splitter/token stream API admits them.
