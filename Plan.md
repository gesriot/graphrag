...
**Readiness snapshot (reviewed 2026-06-25):** the small-project Python→Rust thesis is proven, and the first real multi-package graph scale probe is now green on vendored `sqlparse` 0.5.5 (~4.1k LOC): 243 entities / 454 relationships / 253 call observations, 229 resolved calls, structural pass rate 1.0, 0 dangling targets, 0 semantic suspicions after the audit heuristic was made import-aware, and a seeded 12/12 manual precision sample including cross-package constructor/module calls. The first scaled component target, `sqlparse.split`, now has a pre-Rust golden contract with 23 cases covering tokenizer/splitter edge behavior. Phase 0 is complete. The Python portion of Phase 1 and the deterministic core of Phase 2 are usable (generic package indexing, provenance, audited call edges, local traversals and context packs), while multi-language semantic analysis and optional LLM overlays remain open. Phase 3 exists as separate CLIs rather than one product surface. The Phase 4 small-project milestone is exceeded by four ports. Phase 5 has cleared graph scale plus pre-Rust contract capture for one scaled component, but the Rust port, engineer-time measurement, property/fuzz coverage, and durable intervention log are still missing. Phases 6–7 are not started in substance. In short: a convincing research prototype with unusually good small-scope evidence and an encouraging first repository-scale graph result, not yet a scalable migration product.
...
1. Treat `sqlparse` as the pinned Phase 5 graph baseline: keep the import-aware `semantic_suspicions==0` audit gate, retain the seeded precision sample, and record any future re-index deltas against snapshot `20260618-151436-ad7b5954`.
2. Treat the `sqlparse.split` behavior contract as the pre-Rust gate: 23 frozen cases covering semicolon splitting, `GO` / `GO 2`, transaction `BEGIN`, procedural `CREATE ... BEGIN ... END`, `CASE`, nested parentheses, comments/whitespace, strings, unicode, strip-semicolon mode, and block-comment edge behavior.
3. Port the scoped component through the existing gates: `__init__.split` → `FilterStack` → `lexer.tokenize` → `StatementSplitter` → optional semicolon stripping → `sql.Statement` stringification. Record context-pack inputs, iterations, failures, engineer time, manual-fix count, and behavioral deltas; add property/fuzz tests where the splitter/token stream API admits them.
4. Keep Rust implementation staged: token type tree + keyword tables first, lexer regex parity second, splitter/filter pipeline third, then golden/port_eval. Note that `keywords.SQL_REGEX` is not fully portable to Rust's standard `regex` crate: it uses lookahead/lookbehind and one backreference, so the port needs either `fancy-regex` for incompatible rules or small hand-written scanners for those cases. Do not widen into full `parse`/grouping or `format`/filters until split is green.
5. Expand only after the first scaled component is green: next to `parse`/grouping or `format`/filters if `sqlparse` remains clean, or to `click` if the next bottleneck should be dynamic decorators/resolver stress rather than port breadth.
6. Use the scaled port's measured bottleneck to choose the following branch: production CLI/MCP and benchmarks if orchestration dominates; semantic overlay if architectural retrieval is insufficient; C/clang only after the Python pipeline survives repository-scale porting.
