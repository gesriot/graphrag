...
Validates already-published ``clang_type_*`` entity evidence and its
``clang_types`` manifest block against the producer contract in
``c_clang_types``. Does **not** invoke Clang, load compile_commands.json,
...
Supported states:
  legacy_absent – no manifest block and no ``clang_type_*`` fields
  off           – ``mode=off`` / ``enabled=false`` with no type fields
  enabled       – ``mode=configured_clang_type_declarations`` fully validated
...
Usage:
    uv run python scripts/c_clang_type_graph_audit.py --graph byog_cjson
    uv run python scripts/c_clang_type_graph_audit.py --graph byog_cjson \\
        --snapshot 20260813-081031-10c2c75b --output type_audit.json
"""
...
from graphrag_code.c_clang_types import (  # type: ignore
    CONFIDENCE_BOUNDARY,
    EXTRACTOR,
    FACT_KIND,
    LIMITATIONS,
...
AUDIT_MODE = "clang_type_graph_integrity"
...
class ClangTypeGraphAuditError(Exception):
...
def audit_rows(
    ...
    *,
    ...
    out: Dict[str, Any] = {
        ...
        "n_type_field_carriers": int(result["n_type_field_carriers"]),
...
def resolve_snapshot(
    ...
    if not graph_root.exists():
        raise ClangTypeGraphAuditError(
    ...
    if snapshot is not None:
        ...
        if not snap_id:
            raise ClangTypeGraphAuditError("empty --snapshot id")
...
def format_report(report: Mapping[str, Any]) -> str:
    lines = [
        "Clang type-declaration graph integrity audit "
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    except ClangTypeGraphAuditError as e:
        print(f"c_clang_type_graph_audit: {e}", file=sys.stderr)
        return 2
    ...
    if args.output is not None:
        try:
            ...
            print(
                f"c_clang_type_graph_audit: cannot write output: {e}",
                file=sys.stderr,
            )
            return 2
    if args.json:
