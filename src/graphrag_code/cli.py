...
def components(
    ...
    max_components: int = typer.Option(
        DEFAULT_COMPONENTS_MAX_COMPONENTS, "--max-components"
    ),
    max_nodes_per_component: int = typer.Option(
        DEFAULT_COMPONENTS_MAX_NODES_PER_COMPONENT, "--max-nodes-per-component"
    ),
    edge_type: list[str] = typer.Option(
        [],
        "--edge-type",
...
def strong_components(
    ...
    runtime recursion/deadlock proof, or a UI. ``--dot`` is Graphviz DOT
...
def dependency_order(
    ...
    list; not an MCP tool. ``--dot`` is Graphviz DOT interchange on
    stdout: it does not invoke Graphviz, render an image, or provide an
    interactive UI. ``--json`` and ``--dot`` are mutually exclusive.
    """
