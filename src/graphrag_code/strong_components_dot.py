"""Deterministic Graphviz DOT serialization for a bounded strong-components result.

This module renders an already-computed ``ByogGraph.strong_components`` /
``compute_strongly_connected_components`` mapping. It does not open a
graph, inspect a snapshot, resolve endpoints, traverse or reconstruct
...
relationship rows or cross-component edge records, so this renderer
emits no edge statements and does not invent edges. Internal, cross,
self-loop, and cyclic totals are metadata only.

DOT here is a visualization **interchange** format on stdout. The project
does not render an image and does not provide an interactive UI. Use
``condensation --dot`` when a bounded cross-SCC DAG is required.

Contract
========

* Directed, non-strict ``digraph graphrag_strong_components``.
* Each returned SCC is one Graphviz cluster in **producer order**:
...
* No relationship-edge or cross-component-edge statements.
...
* Presentation-only attributes ``rankdir=LR`` and ``shape=box`` are
  ...
  leadership, severity, or semantic meaning.
* Hard limit: :data:`HARD_MAX_STRONG_COMPONENTS_DOT_BYTES` UTF-8 bytes
...
flags) renders as a valid empty digraph with graph-level metadata and no
...
STRONG_COMPONENTS_DOT_SCHEMA_VERSION = 1
HARD_MAX_STRONG_COMPONENTS_DOT_BYTES = 1_000_000
_GRAPH_NAME = "graphrag_strong_components"
...
class StrongComponentsDotError(ValueError):
    """Invalid strong-components DOT input, unencodable value, or overflow."""


def dumps_strong_components_dot(result: Mapping[str, Any]) -> str:
    """Serialize a strong-components producer result to deterministic DOT.

    The returned string is the complete payload, including the final
    newline, and is guaranteed to be at most
    :data:`HARD_MAX_STRONG_COMPONENTS_DOT_BYTES` UTF-8 bytes.
    """
    try:
        return _dumps_strong_components_dot(result)
    except SubgraphDotError as exc:
        raise StrongComponentsDotError(str(exc)) from exc


def _dumps_strong_components_dot(result: Mapping[str, Any]) -> str:
    if not isinstance(result, Mapping):
        raise StrongComponentsDotError(
            "strong-components DOT input must be a mapping"
        )
    for name in _REQUIRED_TOP:
        if name not in result:
            raise StrongComponentsDotError(f"missing strong-components field {name}")
    ...
    if n_components_returned != len(components):
        raise StrongComponentsDotError(
            "n_components_returned does not match components"
        )
    if n_components_returned > n_components_total:
        raise StrongComponentsDotError("returned component count exceeds total")
    if n_components_returned > max_components:
        raise StrongComponentsDotError(
            "returned component count exceeds max_components"
        )
    ...
    if n_self_loop_edges_total > n_internal_edges_total:
        raise StrongComponentsDotError(
            "self-loop total exceeds internal-edge total"
        )
    ...
    if n_components_total > 0 and n_nodes_total < n_components_total:
        raise StrongComponentsDotError("node total is smaller than component total")
    ...
    graph_attrs: List[Tuple[str, str]] = [
        ("schema_version", str(STRONG_COMPONENTS_DOT_SCHEMA_VERSION)),
    ...
    for index, component in enumerate(components):
        if not isinstance(component, Mapping):
            raise StrongComponentsDotError(
                "strong-components component must be a mapping"
            )
        for name in _REQUIRED_COMPONENT:
            if name not in component:
                raise StrongComponentsDotError(f"missing component field {name}")
        ...
        if representative in seen_representatives:
            raise StrongComponentsDotError(
                f"duplicate strong-components representative {representative!r}"
            )
        ...
        for title in nodes:
            if title in seen_titles:
                raise StrongComponentsDotError(
                    f"duplicate strong-components node title {title!r}"
                )
    ...
    if returned_node_titles > n_nodes_total:
        raise StrongComponentsDotError("returned node titles exceed n_nodes_total")
...
def _require_empty_result(
    *,
    ...
    if components:
        raise StrongComponentsDotError(
            "empty strong-components must have empty material"
        )
    zeros = (
        n_components_returned,
        n_nodes_total,
    ...
    if any(value != 0 for value in zeros):
        raise StrongComponentsDotError(
            "empty strong-components must have zero totals"
        )
    if components_truncated or nodes_truncated:
        raise StrongComponentsDotError(
            "empty strong-components must not be truncated"
        )


def _checked_payload(payload: str) -> str:
    if not isinstance(payload, str):
        raise StrongComponentsDotError("DOT payload must be str")
    ...
    try:
        ...
        raise StrongComponentsDotError(
            "strong-components DOT is not strict UTF-8"
        ) from exc
    if b"\r" in encoded:
        raise StrongComponentsDotError(
    ...
    if any(byte < 0x20 and byte != 0x0A or byte == 0x7F for byte in encoded):
        raise StrongComponentsDotError("DOT payload contains a raw control character")
    if len(encoded) > HARD_MAX_STRONG_COMPONENTS_DOT_BYTES:
        raise StrongComponentsDotError(
            "strong-components DOT exceeds hard limit of "
            f"{HARD_MAX_STRONG_COMPONENTS_DOT_BYTES} bytes"
        )
    return payload
...
def _attr(name: str, value: str) -> str:
    if not name.isidentifier() or not name.isascii():
        raise StrongComponentsDotError(f"invalid DOT attribute name {name!r}")
...
def _edge_types_token(raw: Any) -> str:
    ...
    if not isinstance(raw, list):
        raise StrongComponentsDotError("edge_types must be a list or null")
    if not raw:
        raise StrongComponentsDotError("edge_types must be non-empty when present")
    tokens: List[str] = []
    seen: set[str] = set()
    for item in raw:
        token = _require_nonempty_str(item, "edge type")
        if token.strip() != token or "\x00" in token:
            raise StrongComponentsDotError(f"invalid edge_types value {token!r}")
        if token in seen:
            raise StrongComponentsDotError(f"duplicate edge_types value {token!r}")
        seen.add(token)
        tokens.append(token)
...
def _json_array(values: Sequence[str]) -> str:
    try:
        ...
        raise StrongComponentsDotError("canonical JSON token is invalid") from exc


def _require_list(raw: Any, name: str) -> List[Any]:
    if not isinstance(raw, list):
        raise StrongComponentsDotError(f"{name} must be a list")
    return raw
...
def _require_str_list(raw: Any, name: str) -> List[str]:
    ...
    for item in items:
        ...
        if title in seen:
            raise StrongComponentsDotError(f"duplicate {name} value {title!r}")
        seen.add(title)
        titles.append(title)
    return titles
...
def _require_bool(raw: Any, name: str) -> bool:
    if not isinstance(raw, bool):
        raise StrongComponentsDotError(f"{name} must be a boolean")
    return raw
...
def _require_int(
    ...
    *,
    ...
    if isinstance(raw, bool) or not isinstance(raw, int):
        raise StrongComponentsDotError(f"{name} must be an integer")
    if raw < minimum:
        raise StrongComponentsDotError(f"{name} must be >= {minimum}")
    if maximum is not None and raw > maximum:
        raise StrongComponentsDotError(f"{name} must be <= {maximum}")
    return raw


def _require_str(raw: Any, name: str) -> str:
    if not isinstance(raw, str):
        raise StrongComponentsDotError(f"{name} must be a string")
    try:
        raw.encode("utf-8")
    except UnicodeEncodeError as exc:
        raise StrongComponentsDotError(f"{name} is not strict UTF-8") from exc
    return raw
...
def _require_nonempty_str(raw: Any, name: str) -> str:
    ...
    if not text:
        raise StrongComponentsDotError(f"{name} must be a non-empty string")
    return text
