...
from graphrag_code.audit_call_edges import build_report  # noqa: E402
from graphrag_code.byog_graph import ByogGraph  # noqa: E402
from graphrag_code.paths import require_source_checkout, source_checkout_root

ROOT = source_checkout_root()
DEFAULT_GATE_MANIFEST = (
    (ROOT / "scripts" / "port_gates.json")
    if ROOT is not None
    else Path("scripts/port_gates.json")
)


def _repo() -> Path:
    """Repository root for checkout-only port-eval evidence."""
    if ROOT is not None:
        return ROOT
    return require_source_checkout("port-eval repository evidence")
...
def eval_graph(graph: Path, source: Path, reindex: bool, use_advanced: bool) -> Dict[str, Any]:
    ...
    if reindex:
        cmd = [
            sys.executable, str(_repo() / "scripts" / "index_python.py"),
            "--package", str(source), "--graph", str(graph),
        ]
        if use_advanced:
            cmd.append("--use-advanced")
        reindex_result = _run(cmd, cwd=_repo())
...
def gen_context_packs(
    ...
    *,
    ...
    pack_script = _repo() / "scripts" / "context_pack.py"
    generated: List[str] = []
...
def load_gate_manifest(path: Path) -> Dict[str, Dict[str, Any]]:
    ...
    cargo_ports = {
        str(cargo.parent.relative_to(_repo()))
        for cargo in (_repo() / "examples").glob("*_rust/Cargo.toml")
    }
    declared_ports = {
        str(entry["port"])
    ...
    contract_sources = {
        pkg.name
        for pkg in (_repo() / "examples").iterdir()
        if pkg.is_dir()
...
def _tool_probe(tool: str) -> Dict[str, Any]:
    ...
    if tool == "c_compiler":
        for candidate in ("cc", "gcc", "clang"):
            ...
            if executable is not None:
                result = _run([executable, "--version"], _repo())
                if result["status"] == "ok":
    ...
    if tool == "miri":
        ...
        if cargo["status"] != "ok":
            return cargo
        result = _run(["cargo", "+nightly", "miri", "--version"], _repo())
        if result["status"] == "ok":
...
def _expand_gate_command(command: List[str]) -> List[str]:
    ...
    replacements = {"{python}": sys.executable, "{root}": str(_repo())}
...
def _index_gate(gate: Dict[str, Any], graph: Path) -> Dict[str, Any]:
    ...
    source = _repo() / gate["source"]
    if gate["indexer"] == "python":
        command = [
            sys.executable,
            str(_repo() / "scripts" / "index_python.py"),
            "--package",
            str(source),
            "--graph",
            str(graph),
...
def run_declared_gate(
    ...
    graph = _repo() / "output" / "port_gates" / ident / "graph"
...
def main(
    ...
    if gate or all_gates:
        ...
        manifest_path = gate_manifest if gate_manifest.is_absolute() else _repo() / gate_manifest
        try:
    ...
    report = build_eval_report(
        ...
        port_dir=port if port.is_absolute() else Path.cwd() / port,
        graph=graph if graph.is_absolute() else Path.cwd() / graph,
