...
Validates already-published ``clang_call_*`` relationship fields and the
``clang_calls`` manifest block against the producer contract in
``c_clang_calls``. Does **not** invoke Clang, load compile_commands.json,
read C sources, reconstruct byte offsets, reindex, publish, re-run the
overlay, rewrite byog_* roots, or repair rows.

Supported states:
  legacy_absent – no manifest block and no ``clang_call_*`` fields
  off           – ``mode=off`` / ``enabled=false`` with no call fields
  enabled       – ``mode=clang_configured_call_overlay`` fully validated
...
Usage:
    uv run python scripts/c_clang_call_graph_audit.py --graph byog_cjson
    uv run python scripts/c_clang_call_graph_audit.py --graph byog_cjson \\
        --snapshot 20260808-153310-5b68f044 --output call_audit.json
"""
...
from graphrag_code.c_clang_calls import (  # type: ignore
    CONFIDENCE_BOUNDARY,
    EXTRACTOR,
    FACT_KIND,
    LIMITATIONS,
...
AUDIT_MODE = "clang_call_graph_integrity"
...
class ClangCallGraphAuditError(Exception):
...
def audit_rows(
    ...
    *,
    ...
    out: Dict[str, Any] = {
        ...
        "n_call_field_carriers": int(result["n_call_field_carriers"]),
...
def resolve_snapshot(
    ...
    if not graph_root.exists():
        raise ClangCallGraphAuditError(f"graph root does not exist: {graph_root}")
    snapshots_dir = (graph_root / "snapshots").resolve()
    if snapshot is not None:
        snap_id = snapshot.strip()
        if not snap_id:
            raise ClangCallGraphAuditError("empty --snapshot id")
        if Path(snap_id).name != snap_id or snap_id in {".", ".."}:
            raise ClangCallGraphAuditError(f"unsafe snapshot id: {snap_id!r}")
        snap_dir = (snapshots_dir / snap_id).resolve()
        if snap_dir.parent != snapshots_dir:
            raise ClangCallGraphAuditError(
                f"snapshot escapes snapshots directory: {snap_id!r}"
            )
        if not snap_dir.is_dir():
            raise ClangCallGraphAuditError(f"snapshot directory missing: {snap_dir}")
    else:
        ...
        if pointer.is_file():
            ...
            if not snap_id:
                raise ClangCallGraphAuditError(f"empty current pointer: {pointer}")
            if Path(snap_id).name != snap_id or snap_id in {".", ".."}:
                raise ClangCallGraphAuditError(
...
def _load_relationships(snap_dir: Path) -> List[Dict[str, Any]]:
    path = snap_dir / "relationships.parquet"
    if not path.is_file():
        raise ClangCallGraphAuditError(
            f"snapshot missing relationships.parquet: {path}"
        )
    try:
        import pandas as pd
    ...
    try:
        ...
        raise ClangCallGraphAuditError(
            f"cannot read relationships.parquet: {path}: {e}"
        ) from e
    try:
        return df.to_dict("records")
    except Exception as e:
        raise ClangCallGraphAuditError(
            f"cannot materialize relationship rows: {e}"
        ) from e
...
def read_only_fingerprint(graph_root: Path, snap_dir: Path) -> Dict[str, str]:
    ...
    for name in _READ_ONLY_FILES:
        ...
        try:
            ...
            raise ClangCallGraphAuditError(f"cannot fingerprint {path}: {e}") from e
    ...
    if snapshots_dir.is_dir():
        try:
            ...
            raise ClangCallGraphAuditError(f"cannot list {snapshots_dir}: {e}") from e
...
def audit_graph_root(
    ...
    *,
    ...
    relationships = _load_relationships(snap_dir)
    report = audit_rows(
        relationships,
        manifest,
        max_anomaly_samples=max_anomaly_samples,
...
def format_report(report: Mapping[str, Any]) -> str:
    lines = [
        "Clang configured-call graph integrity audit "
        "(read-only; no Clang re-run)",
    ...
    lines.append(
        ...
        f"calls={report.get('n_calls')} anomalies={report.get('n_anomalies')}"
        + (
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    except ClangCallGraphAuditError as e:
        print(f"c_clang_call_graph_audit: {e}", file=sys.stderr)
        return 2
    ...
    if args.output is not None:
        try:
            ...
            print(
                f"c_clang_call_graph_audit: cannot write output: {e}",
                file=sys.stderr,
            )
            return 2
    if args.json:
