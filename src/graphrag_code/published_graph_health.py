...
ROOT = source_checkout_root() or Path.cwd()
...
def load_specs(manifest: Path = DEFAULT_MANIFEST) -> list[PublishedGraphSpec]:
    ...
    from graphrag_code.port_eval import load_gate_manifest  # type: ignore
...
def _fresh_data(spec: PublishedGraphSpec, root: Path) -> dict[str, list[dict[str, Any]]]:
    ...
    if spec.indexer == "c":
        from graphrag_code.c_preprocessor import annotate_byog  # type: ignore
...
def check_spec(
    ...
    *,
    ...
    from graphrag_code.persisted_graph_integrity import (  # type: ignore
