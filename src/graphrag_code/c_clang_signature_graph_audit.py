...
Validates already-published ``clang_signature_*`` / ``clang_qual_type`` entity
evidence and its ``clang_signatures`` manifest block against the producer
contract in ``c_clang_signatures``. Does **not** invoke Clang, load
compile_commands.json, build an AST capture, reindex, publish, re-run the
overlay, rewrite byog_* roots, or create compiler/AST artifacts. Nothing is
ever repaired.

Supported states:
  legacy_absent – no manifest block and no signature fields
  off           – ``mode=off`` / ``enabled=false`` with no signature fields
  enabled       – ``mode=clang_ast_signatures`` fully validated
...
Usage:
    uv run python scripts/c_clang_signature_graph_audit.py --graph byog_cjson
    uv run python scripts/c_clang_signature_graph_audit.py --graph byog_cjson \\
        --snapshot 20260808-153310-5b68f044 --output sig_audit.json
"""
...
from graphrag_code.c_clang_signatures import (  # type: ignore
    CONFIDENCE_BOUNDARY,
    EXTRACTOR,
    FACT_KIND,
    LIMITATIONS,
...
AUDIT_MODE = "clang_signature_graph_integrity"

# Inputs whose bytes must be identical before and after the audit.
# BYOG tables (not GraphRAG nodes/edges/communities) are the persisted graph.
_READ_ONLY_FILES = (
    "manifest.json",
    "entities.parquet",
...
class ClangSignatureGraphAuditError(Exception):
...
def audit_rows(
    ...
    *,
    ...
    out: Dict[str, Any] = {
        ...
        "n_signature_field_carriers": int(result["n_signature_field_carriers"]),
...
def resolve_snapshot(
    ...
    if not graph_root.exists():
        raise ClangSignatureGraphAuditError(
    ...
    if snapshot is not None:
        ...
        if not snap_id:
            raise ClangSignatureGraphAuditError("empty --snapshot id")
...
def format_report(report: Mapping[str, Any]) -> str:
    lines = [
        "Clang function-signature graph integrity audit "
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    except ClangSignatureGraphAuditError as e:
        print(f"c_clang_signature_graph_audit: {e}", file=sys.stderr)
        return 2
    ...
    if args.output is not None:
        try:
            ...
            print(
                f"c_clang_signature_graph_audit: cannot write output: {e}",
                file=sys.stderr,
            )
            return 2
    if args.json:
