...
Validates already-published ``preprocessor_*`` row stamps and the
``preprocessor_liveness`` manifest block against the producer contract in
``c_preprocessor``. Does **not** invoke a compiler, read compile_commands.json,
read C/header sources, reconstruct macro tables or branch decisions, compare
the recorded digest with the current host, reindex, restamp, publish, rewrite
byog_* roots, or repair rows.

Supported states:
  legacy_absent      – no manifest block and no material stamp evidence
  no_compiler        – host-independent stamps, internally consistent
  compiler_builtins  – host-specific stamps, internally consistent
...
Usage:
    uv run python scripts/c_preprocessor_liveness_graph_audit.py --graph byog_cjson
    uv run python scripts/c_preprocessor_liveness_graph_audit.py --graph byog_cjson \\
        --snapshot 20260808-153310-5b68f044 --output liveness_audit.json
"""
...
from graphrag_code.c_preprocessor import (  # type: ignore
    LIMITATIONS,
    strict_json_loads,
    validate_persisted_preprocessor_liveness,
)

AUDIT_MODE = "preprocessor_liveness_graph_integrity"
...
class PreprocessorLivenessGraphAuditError(Exception):
...
def audit_rows(
    ...
    *,
    ...
    """Pure dataframe/row API. Never mutates inputs or reanalyses sources."""
    ...
    out: Dict[str, Any] = {
        ...
        "observations_present": bool(result["observations_present"]),
...
def resolve_snapshot(
    ...
    if not graph_root.exists():
        raise PreprocessorLivenessGraphAuditError(
    ...
    if snapshot is not None:
        ...
        if not snap_id:
            raise PreprocessorLivenessGraphAuditError("empty --snapshot id")
...
def _load_table(snap_dir: Path, name: str, *, required: bool) -> Optional[List[Dict[str, Any]]]:
...
def format_report(report: Mapping[str, Any]) -> str:
    lines = [
        "C preprocessor-liveness graph integrity audit "
        "(read-only; no reanalyse)",
    ...
    lines.append(
        f"  stamped_rows={report.get('n_stamped_rows')} "
        f"observations={report.get('n_call_observations')} "
    ...
    if provenance.get("macro_seed_digest"):
        lines.append(f"  digest={provenance.get('macro_seed_digest')}")
    ...
    for anomaly in report.get("anomalies") or []:
        loc = ""
        if anomaly.get("table") is not None:
            loc += f" {anomaly.get('table')}"
        if anomaly.get("row_index") is not None:
            loc += f"[{anomaly.get('row_index')}]"
        lines.append(
            f"  - [{anomaly.get('code')}]{loc}: {anomaly.get('message')}"
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    except PreprocessorLivenessGraphAuditError as e:
        print(f"c_preprocessor_liveness_graph_audit: {e}", file=sys.stderr)
        return 2
    ...
    if args.output is not None:
        try:
            ...
            print(
                f"c_preprocessor_liveness_graph_audit: cannot write output: {e}",
                file=sys.stderr,
            )
            return 2
    if args.json:
