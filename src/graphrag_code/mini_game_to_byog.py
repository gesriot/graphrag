...
PACKAGE_DIR = (
    (_CHECKOUT / "examples" / "mini_game") if _CHECKOUT is not None else Path("examples/mini_game")
)
OUT_ROOT = (_CHECKOUT / "byog_mini_game") if _CHECKOUT is not None else Path("byog_mini_game")
...
def build_byog_for_package(use_advanced: bool = False, package_dir: Path | None = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        from graphrag_code.python_dynamic import annotate_byog  # type: ignore
