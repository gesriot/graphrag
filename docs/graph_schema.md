...
it is intentionally absent from MCP. The fixed MCP tool set remains 17
read-only tools.
...
retention guarantee. MCP remains strict and remains exactly 17 read-only tools.
...
| MCP | Read-only tool, registered immediately after `neighbors`. Envelope `data` is the exact `ByogGraph.subgraph` result. `truncated` is `nodes_truncated or edges_truncated`; `total` / `returned` are the sums of the node and edge counts. Limits include the validated `direction`, `max_depth`, `max_nodes`, `max_edges`, `edge_types`, and `max_envelope_bytes`. The producer is the only truncation source. The 1 MiB envelope limit fails closed. MCP always uses `allow_unlocked_managed=False`. MCP does not expose DOT or a format parameter; the fixed surface remains exactly 17 tools |
...
MCP. The fixed MCP tool set remains 17 read-only tools.
...
from MCP. The fixed MCP tool set remains 17 read-only tools.
...
MCP tool set remains 17 read-only tools.
...
intentionally absent from MCP. The fixed MCP tool set remains 17
read-only tools.
...
tool set remains 17 read-only tools.
...
absent from MCP. The fixed MCP tool set remains 17 read-only
tools.
...
The fixed MCP tool set remains 17 read-only tools.
...
set remains 17 read-only tools.
...
17 read-only tools. `--transfer-confirmed` is mandatory.
