...
not invoked. MCP exposes the existing bounded producer as
`impact_graph`, the 18th read-only tool, immediately after legacy
`impact` and immediately before `type_closure`. There is no hyphenated
alias. MCP does not expose DOT or output-format selection.
