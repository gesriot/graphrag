...
Validates already-published ``depends_on`` / ``translation_unit_dependency``
relationships and the ``compiler_dependencies`` manifest block against the
producer contract in ``c_compiler_facts``. Does **not** invoke a compiler,
read compile_commands.json, read C/header sources, run ``compiler -M``,
reconstruct dependency sets, reindex, publish, rewrite byog_* roots, or
repair rows.

Supported states:
  legacy_absent – no manifest block and no dependency-overlay evidence
  off           – ``mode=off`` / ``enabled=false`` with no dependency edges
  enabled       – ``mode=compiler_m`` fully validated
...
Usage:
    uv run python scripts/c_compiler_dependency_graph_audit.py --graph byog_cjson
    uv run python scripts/c_compiler_dependency_graph_audit.py --graph byog_cjson \\
        --snapshot 20260808-153310-5b68f044 --output dep_audit.json
"""
...
AUDIT_MODE = "compiler_dependency_graph_integrity"
...
class CompilerDependencyGraphAuditError(Exception):
...
def audit_rows(
    ...
    *,
    ...
    out: Dict[str, Any] = {
        ...
        "n_dependency_carriers": int(result["n_dependency_carriers"]),
...
def resolve_snapshot(
    ...
    if not graph_root.exists():
        raise CompilerDependencyGraphAuditError(
    ...
    if snapshot is not None:
        ...
        if not snap_id:
            raise CompilerDependencyGraphAuditError("empty --snapshot id")
...
def format_report(report: Mapping[str, Any]) -> str:
    lines = [
        "Compiler TU-dependency graph integrity audit "
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    except CompilerDependencyGraphAuditError as e:
        print(f"c_compiler_dependency_graph_audit: {e}", file=sys.stderr)
        return 2
    ...
    if args.output is not None:
        try:
            ...
            print(
                f"c_compiler_dependency_graph_audit: cannot write output: {e}",
                file=sys.stderr,
            )
            return 2
    if args.json:
