...
5. **Local queries and context packs** – `scripts/graph_query.py` (callers,
   ...
   MCP remains exactly 19 read-only
   ...
   material only. Producer order is preserved. There is no
   `call_observations` alias, hyphenated alias, or DOT. Empty results
   do not prove runtime absence. This is not a runtime trace, complete
   dynamic-dispatch reconstruction, proof that an unresolved call
   ...
   `degree-ranking` is raw directed relationship-row degree accounting
   (`--rank-by total|incoming|outgoing`), not PageRank, betweenness,
   ...
   or natural-language analysis. MCP remains exactly 19 read-only tools.
   ...
   remains exactly 19 read-only tools. `condensation` is a bounded directed
   ...
   alias. MCP remains exactly 19 read-only tools. `shortest-path` is a
   ...
   MCP remains exactly 19 read-only tools. `type-closure` is a bounded
   ...
   `--dot` is a deterministic Graphviz DOT interchange of that same
   producer mapping on stdout; Graphviz is not invoked. Independent
   node/edge caps may emit edge-only endpoint nodes with `in_nodes=false`
   ...
   MCP remains exactly 19 read-only tools. `impact` is one canonical
   ...
   or the unbounded `impact` title list. MCP remains exactly 19
   read-only tools.
...
`snapshot_history` and `snapshot_diff` to the fixed 14-tool set and stays
...
`.publish.lock`, and is intentionally absent from the fixed 14-tool MCP
...
CLI; MCP `snapshot="current"` on the existing twelve selectable tools).
...
for an explicit published id. The MCP tool set remains exactly 19.
...
MCP remains exactly 19 read-only tools. ``graphrag-code
...
``graphrag-code snapshot-transfer-plan`` (also ``python -m
graphrag_code.snapshot_transfer_plan`` and
``scripts/snapshot_transfer_plan.py``) is a read-only plan for
transferring one retained snapshot from one managed graph to a
different managed graph without first creating a standalone
export. It does not export, import, copy, activate, or mutate
either tree. Same-graph identity is rejected before nested
